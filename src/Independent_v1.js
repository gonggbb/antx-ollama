...
const Independent = () => {
  ...
  const onSubmit = (nextContent) => {
    ...
    setMessagesMap(prev => addMessageToMap(prev, activeKey, { id: newKey, content: nextContent, role: 'local' }));
    setContent('');
  };

  const onAddConversation = () => {
    const newKey = `${conversationsItems.length}`;
    setConversationsItems([...conversationsItems, { key: newKey, label: `New Conversation ${conversationsItems.length}` }]);
    setActiveKey(newKey);
  };
